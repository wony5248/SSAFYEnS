...
const Daily = () =>{
    ...
    useEffect(() => {
        ...
        async function getMonthlySchedule() {
            try{
                const result = await scheduleAPI.getWeeklyAverage(moment(date).format('YYYY-MM-DD'));
                setData(result.data);
                let s = moment(date).startOf('week');
                let c = [];
                let t = [];
                let dic = {};
                dic[s.format('MM-DD')]=0;
                c[0] = s.format('MM-DD');
                t[0] = 0
                let text = [];
                for (let i = 1; i < 7; i++){
                    c[i] = s.add(1, "day").format('MM-DD');
                    dic[s.format('MM-DD')] = i;
                    t[i] = 0;
                }
                for(let i=0; i<result.data.schedules.length; i++){
                    if(result.data.schedules[i].is_finished===1){
                        t[dic[moment(result.data.schedules[i].started_at).format('MM-DD')]] += moment.duration(moment(result.data.schedules[i].finished_at).diff(moment(result.data.schedules[i].started_at))).asMinutes();
