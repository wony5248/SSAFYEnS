...
const Daily = () =>{
    ...
    useEffect(() => {
        let completed = false;
        let start = moment(date).startOf('month').week();
        let end = moment(date).endOf('month').week();
        async function getMonthlySchedule() {
            try{
                const result = await scheduleAPI.getMonthlyAverage(moment(date).format('YYYY-MM-DD'));
                setData(result.data);
                let c = [];
                let t = [];
                let dic = {};
                ...
                for(let i=0; i<result.data.schedules.length; i++){
                    t[dic[Number(result.data.schedules[i].week)]] += moment.duration(moment(result.data.schedules[i].finished_at).diff(moment(result.data.schedules[i].started_at))).asMinutes();
